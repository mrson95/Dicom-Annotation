...
class AppWidgets(QMainWindow):
    ...
    def showDialog(self):
        ...
        num = 0
        com = []
        fname = QFileDialog.getOpenFileName(self, 'Open file', 'images/', '*.dcm')
    ...
    def annotation_button_on_click(self):
        ...
        if self.annotation_button_status == 1:
            self.annotation_button.setText('Annotation On')
            self.annotation_button.setStyleSheet('QPushButton {color: red;}')
        
        else:
            self.annotation_button.setText('Annotation Off')
            self.annotation_button.setStyleSheet('QPushButton {color: black;}')


        self.update_display()

        if self.annotation_button_status == 1:
            print('button on : ', self.annotation_button_status)

        else:
            print('button off : ', self.annotation_button_status)
