...
class SpineLabelingApp(AppLayouts, InfoStorageDicts, QDialog):
        ...
        if event.inaxes==self.axes:
            if event.button == 1:
                if not event.dblclick:
                    if not self.block_points and self.mode == self.nr.edit and self.points_off_on_status == 2 and AppWidgets.link_annotation_status(self) == 0:
                        last_time = self.now
                        self.now = time.time()
                        if self.time_log_flag:
                            with open(self.time_log_file,  'a+') as file:
                        ...
                        self.StoreDict[cur_imgID][cur_VB][self.CoordType] = (self.co_x, self.co_y)
                        self.last_labeled = (cur_VB, self.CoordType)
                        ...
                        def dialog_open(self):
                            ...
                            button_layout = QHBoxLayout()
                            self.corDiaLayout = QVBoxLayout()
                            self.corDiaLayout.addLayout(button_layout)
                            ...
                            save_button.clicked.connect(self.dialog_save)
                        ...
                        if (len(self.x_list) > 3) and (len(self.y_list) > 3):
                            ...
                            if dis_x and dis_y:
                                ...
                                self.x_list = self.x_list[1:-1]
                                self.y_list = self.y_list[1:-1]

                                self.x_list.append(int(self.cross_x))
                                self.x_list.insert(0, int(self.cross_x))
                                ...
                                for k in range(0, len(self.x_list)):
                                    self.only_x_list.append(self.x_list[k])
                                    self.only_y_list.append(self.y_list[k])
                                    
                                self.only_x_list.sort()
                                self.only_y_list.sort()

                                self.min_x = self.only_x_list[0]
                                self.max_x = self.only_x_list[-1]

                                self.min_y = self.only_y_list[0]
                                self.max_y = self.only_y_list[-1]
    ...
    def dialog_save(self):
        ...
        self.result_file_name = 'results_' + self.file_name
        self.result_file_path = self.file_path + self.result_file_name

        self.ds.save_as(self.result_file_path)
    ...
    def dialog_ok(self):
        ...
        for i in range(self.min_y, self.max_y):
            for j in range(self.min_x, self.max_x):
                count = 0
                for s in range(1, len(self.x_list)):
                    if (self.x_list[s-1] > j) != (self.x_list[s] > j):
                        y = ((self.y_list[s] - self.y_list[s-1]) / (self.x_list[s] - self.x_list[s-1]))*(j - self.x_list[s-1]) + self.y_list[s-1]
                        
                        if i < y:
                            count += 1
                
                if (count % 2) == 1:
                    self.ds_pixarr[j][i] = int(self.pix_data)
                    count = 0
